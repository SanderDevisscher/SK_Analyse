...
app_data_prepped <- app_data %>% 
  ...
  select(link, Location, Recorder, Datum, Jaar, L00, L0, L1, L2, M1, M2, AM, AV, -contains("fuiken")) %>% 
  group_by(link) %>% 
  mutate(Totaal = sum(L00, L0, L1, L2, M1, M2, AM, AV, na.rm = TRUE),
         Totaal_Larven.All = sum(L00, L0, L1, L2, na.rm = TRUE),
         Totaal_Larven.CPUE = sum(L0, L1, L2, na.rm = TRUE),Totaal_Juvenielen = sum(M1, M2, na.rm = TRUE),
...
write_delim(all_data, "./Intermediate/Brondata_compiled.csv")
```
