...
if(bo_email == ""){
  input <- dlgInput("Your email:")
  bo_email <- input$res
  Sys.setenv(bo_email = bo_email)
}
...
# Data manipulation
```{r import data}
SKData <- read_sheet("1_cBgfSjnxakLAQ9uGunDt_-Mk4L0uDx2-qv6cKCzkSk", 
                                 sheet = "Formulierreacties 1", col_types = "c")

andere_vijvers <- read_sheet("1_cBgfSjnxakLAQ9uGunDt_-Mk4L0uDx2-qv6cKCzkSk", 
                                 sheet = "Andere Vijvers", col_types = "c")
```
...
temp <- temp %>% 
  mutate(Recorder = case_when(grepl("dries jonckers", Recorder, ignore.case = TRUE) ~ "Dries Jonckers",
                              TRUE ~ Recorder))
...
temp <- Records_met_datums %>% 
  mutate(Jaar = case_when(!is.na(Jaar) ~ Jaar,
                          TRUE ~ as.character(lubridate::year(Datum))))
