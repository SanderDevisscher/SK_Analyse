...
GRA_Brondata$Datum2 <- as.factor(format(GRA_Brondata$Datum, format='%m-%d'))
```
...
for(j in Jaren){
  ...
  for(i in Locations){
    ...
    #Create plot
    plot <- ggplot(temp3, aes(x=as.character.Date(Datum2), y=CPUE)) + 
...
for(j in Jaren){
  ...
  for(i in Locations){
    ...
    temp3 <- subset(temp2, Location == i )
    #Create plot
    plot <- ggplot(temp3, aes(x=as.character.Date(Datum2), y=Totaal)) + geom_bar(stat="identity")
    ...
    print(plot)
    #Calculate dynamic dimensions
    width <- ifelse(nrow(temp3)<13, 20, (nrow(temp3)*1.5))
    height<- ifelse(nrow(temp3)<13, 5, (nrow(temp3)*1))
...
for(i in Locations){
  ...
  if(o > 1){
    ...
    plot <- plot + xlab("Datum")
    plot <- plot + ylab("CPUE")
    plot <- plot + facet_wrap(~Jaar, scales = "free_x", nrow = o)
    #Calculate dynamic dimensions
    width <- ifelse(nrow(temp3)<13, 20, (nrow(temp3)*1.5))
    height<- ifelse(nrow(temp3)<13, 5, (nrow(temp3)*1))
...
for(j in Jaren){
  ...
  for(l in Locations){
    for(i in 1:nrow(temp2)){
           if(i == 1){
        temp2$GSL[i] <- temp2$CPUE*26
      }else{
        if(i == 2){
          temp2$GSL[i] <- ((temp2$CPUE[i-1] + temp2$CPUE[i])/2)*26
        }else{
          temp2$GSL[i] <- ((temp2$CPUE[i-1] + temp2$CPUE[i]+temp2$CPUE[i-2])/3)*26
        }
      }
      temp2$Remainder[i] <- temp2$GSL[i+1] + temp2$Totaal_Larven.CPUE[i+1]
      temp2$Error[i] <- abs(temp2$GSL[i]-temp2$Remainder[i])
    }
...
for(j in Jaren){
  ...
  for(l in Locations){
    ...
    #Create plot
    plot <- ggplot(temp3, aes(x=as.character.Date(Datum2), y=GSL)) + geom_bar(stat="identity")
    ...
    plot <- plot + geom_errorbar(aes(ymin=GSL-Error, ymax=GSL+Error), width=.1)
    print(plot)
    #Calculate dynamic dimensions
    width <- ifelse(nrow(temp3)<13, 20, (nrow(temp3)*1.5))
    height<- ifelse(nrow(temp3)<13, 5, (nrow(temp3)*1))
