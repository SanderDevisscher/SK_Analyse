...
for(j in Jaren){
  ...
  for(l in Locations){
    ...
    for(i in 1:nrow(temp2A)){
      ...
      t <- nrow(temp2A)
      p <- i+1
      temp2A$Remainder[i] <- temp2A$GSL[t] + sum(temp2A$Totaal_Larven.CPUE[p:t],na.rm =F)
      temp2A$Error[i] <- temp2A$GSL[i]-temp2A$Remainder[i]
    }
...
for(j in Jaren){
  ...
  for(l in Locations){
    ...
    fNaam <- paste(fNaam, ".jpeg", sep = "")
    #subset locations
    temp5 <- subset(temp4, Location == l)
    #Create plot
    plot <- ggplot(temp5, aes(x=as.character.Date(Datum2), y=GSL)) + geom_bar(stat="identity", fill = "white")
