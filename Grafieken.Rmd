...
for(j in Jaren){
  ...
  for(i in Locations){
    ...
    plot <- plot + ylab("CPUE")
    if(nrow(temp3)>7){
    ggsave(filename = fNaam, path = "./Output/Afbeeldingen/", width=nrow(temp3)*3, height = nrow(temp3)*0.75, units = c("cm"), dpi = 300)
    }else{
    ggsave(filename = fNaam, path = "./Output/Afbeeldingen/", width= 21, height = 5.25, units = c("cm"), dpi = 300)
    }
  }
}
```
...
for(j in Jaren){
  temp2 <- subset(tempgather, Jaar == j)
  Locations <- unique(temp2$Location)
  for(i in Locations){
    temp3 <- subset(temp2, Location == i )
    fNaam <- paste(i ,"TOT", j, sep="_")
    fNaam <- paste(fNaam, ".jpeg", sep = "")
    plot <- ggplot(temp3, aes(x=as.character.Date(Datum), y=Totaal, fill=Stage)) + geom_bar(stat="identity")
    ...
    print(plot)
    if(nrow(temp3)>7){
    ggsave(filename = fNaam, path = "./Output/Afbeeldingen/", width=nrow(temp3)*3, height = nrow(temp3)*0.75, units = c("cm"), dpi = 300, limitsize = F)
    }else{
    ggsave(filename = fNaam, path = "./Output/Afbeeldingen/", width= 21, height = 5.25, units = c("cm"), dpi = 300)
    }
  }
...
for(i in Locations){
  ...
  if(o > 1){
    ...
    plot <- plot + xlab("Datum")
    plot <- plot + ylab("CPUE")
    width <- ifelse(nrow(temp3)<7, 20, (nrow(temp3)*3))
    height<- ifelse(nrow(temp3)<7, 5, (nrow(temp3)*0.75))
    print(paste("w:",width))
...
for(i in Locations){
  ...
  if(o > 1){
    ...
    print(paste("w:",width))
    print(paste("h:",height))
    ggsave(fNaam, path= "./Output/Afbeeldingen/", width=width, height=height, units = c("in"), dpi = 300, limitsize = F)
  }
  else{
    print(paste(i, "heeft slechts 1 jaar data"))}
}
