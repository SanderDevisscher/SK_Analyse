...
for(j in Jaren){
  ...
  for(i in Locations){
    ...
    plot <- plot + scale_x_discrete(expand = c(0, 0), wrap_format(width = 5)) + scale_y_continuous(limits=c(0,NA),expand = c(0, 0))
    plot <- plot + theme(legend.position="none")
    plot <- plot + theme(axis.line = element_line(color="black", size = 0.5))
    plot <- plot + xlab("Datum")
    plot <- plot + ylab("CPUE")
    print(plot)
    ggsave(filename = fNaam, path = "./Output/", width=10.5, height=5, units = c("in"), dpi = 300)
  }
}
```
