...
Arendonk_Overview_Data <- Afvangsten %>% 
  ...
  group_by(Jaar, Location) %>% 
  summarise(Totale_vangst = sum(Totaal, na.rm = TRUE)) %>% 
  arrange(Jaar, Totale_vangst)

plot <- ggplot(Arendonk_Overview_Data, aes(x = Jaar, y= Totale_vangst, fill = Location)) 
plot <- plot + geom_col(position = "dodge")
plot <- plot + ylab("Aantal gevangen stierkikkers")
plot <- plot + scale_y_continuous(limits=c(0,NA),expand = c(0, 0))
