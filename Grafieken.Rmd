...
write_delim(Afvangsten, "./Intermediate/Afvangsten.csv", ";")
```
...
GRA_Brondata_Long <- GRA_Brondata %>% 
  select(-Totaal, -Totaal_Larven.All, -Totaal_Larven.CPUE, -CPUE) %>% 
  gather(Stadium,n, -Datum, -Location, -`Aantal fuiken (Totaal)`) %>% 
  ...
  summarise(n = sum(n),
            aantal_fuiken = sum(`Aantal fuiken (Totaal)`))
...
for(l in Locations){
  ...
  for(j in jaren){
    ...
    path <- paste("./Output/Afbeeldingen/", i2, sep="")
    dir.create(path)
    fNaam <- paste(l ,"CPUE_per_Stadium", j, sep="_")
    fNaam <- paste(fNaam, ".jpeg", sep = "")
    p <- ggplot(temp2, aes(x = as.factor(Maand), y = n/aantal_fuiken, fill = Stadium)) +
      geom_bar(position = "stack", stat = "identity") +
      xlab("Maand") + 
      ylab("Vangst per unit of effort") +
      ggtitle(label = "CPUE per stadium",
              subtitle = paste(l, "-", j))
    ggsave(filename = fNaam, path = path, width=10, height=10, units = c("cm"), dpi = 300, limitsize = F)
  }
}
