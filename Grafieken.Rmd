...
for(j in Jaren){
  ...
  for(i in Locations){
    ...
    print(plot)
    if(nrow(temp3)>7){
    ggsave(filename = fNaam, path = "./Output/", width=nrow(temp3)*3, height = nrow(temp3)*0.75, units = c("cm"), dpi = 300)
    }else{
    ggsave(filename = fNaam, path = "./Output/", width= 20, height = 5, units = c("cm"), dpi = 300)
    }
  }
}
```
